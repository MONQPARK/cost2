...
// 1. Fix splash cards (add Social Content to the opening splash screen)
const socialSplashCard = `
      <button class="splash-card" type="button" onclick="enterMode('social')">
        <div class="splash-card-icon">🚀</div>
        <h3>소셜 콘텐츠</h3>
        <p>AI 기반 타깃 분석<br>플랫폼별 콘텐츠 자동 생성</p>
      </button>`;
if (!html.includes('onclick="enterMode(\'social\')"')) {
  html = html.replace(/(<button class="splash-card" type="button" onclick="enterMode\('info'\)">[\s\S]*?<\/button>)/, '$1\n' + socialSplashCard);
}

// 2. Inject Social Tab Contents (if missing)
const tabContentsHtml = `
    ...
    <div class="card">
      ...
      <div style="display:grid; grid-template-columns:1fr 1fr; gap:15px;">
        ...
        <div class="form-group" style="display:flex; align-items:flex-end;">
          <button type="button" class="btn" style="width:100%;" onclick="alert('기능 준비중입니다.')">📊 정보트렌드 키워드 가져오기</button>
        </div>
      </div>
      <div class="form-group">
  ...
  <div id="tab-social-insight" class="tab-content">
  ...
  <div id="tab-social-content" class="tab-content">
  ...
  <div id="tab-social-schedule" class="tab-content">
...
if (!html.includes('id="tab-social-input"')) {
  // Insert before the last closing </div> inside app-container or just before the scripts
  html = html.replace(/(<script src="\.\/js\/social-prompts\.js"><\/script>)/, tabContentsHtml + '\n  $1');
}

// 3. Fix switchMode to use .tab-content and not throw error on tab-content ID
...
console.log('Fixed switchMode, injected social content panes, and added splash card.');
